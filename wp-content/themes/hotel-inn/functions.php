...
{
	add_menu_page(
		'Hotel Inn Free',    // Page title
		'Hotel Inn Free',    // Menu title
		'manage_options', // Capability required to access the menu page
		'hotel_inn_free',     // Menu slug (unique identifier)
...
?>
					...
					<div class="card p-lg-4 p-3 m-0">
						<div class="tab-content m-0" id="bl-pills-tabContent">
							<div class="tab-pane fade " id="bl-pills-1" role="tabpanel" aria-labelledby="bl-pills-1-tab">
								<form method="post">
									...
									<div class="show_hide_label">
										<div><label for="hotel_inn_dslc_breadcrumb_show">Show Breadcrumbs on your website</label></div>
										<div class="onoff" id="onoff2">
											<input name="hotel_inn_dslc_breadcrumb_show" type="checkbox" value="1" id="hotel_inn_dslc_breadcrumb_show" <?php echo (get_option('hotel_inn_dslc_breadcrumb_show') === "on") ? "checked" : ""; ?>>
											<label for="hotel_inn_dslc_breadcrumb_show"> </label>
										</div>
									</div>
									...
									<div class="bl_list_">
										<ul class="template_listing" id="template_list">
											...
											?>
												<li>
													<label class="all new">
														<input class="hotel_inn_breadcrumb_post" name="<?php echo 'hotel_inn_dslc_' . $key; ?>" <?php echo ((get_option('hotel_inn_dslc_' . $key) == 1) ? "checked" : ""); ?> type="checkbox"><?php echo $item ?>
													</label>
												</li>
										...
										<label class="bread_all">
											<input class="Select All" name="hotel_inn_dslc_bread_all" <?php echo ((get_option('hotel_inn_dslc_bread_all') == 1) ? "checked" : "") ?> type="checkbox">Apply to all
										</label>
									</div>
							...
							<div class="tab-pane fade show active" id="bl-pills-2" role="tabpanel" aria-labelledby="bl-pills-2-tab">
								<form method="post">
									<input type="hidden" value="1" name="hotel_inn_free_back_top_top_enable_form">
									<div class="show_hide_label">
										<div><label for="back_top_top_enable">Show Back To Top button on your website</label></div>
										<div class="onoff">
											<input type="checkbox" id="back_top_top_enable" name="back_top_top_enable" <?php if (get_option('hotel_inn_free_back_top_top_enable') && get_option('hotel_inn_free_back_top_top_enable') == "on") {
																															echo "checked";
																														} ?>>
							...
							<div class="tab-pane fade" id="bl-pills-3" role="tabpanel" aria-labelledby="bl-pills-3-tab">
								<div>
									<a href="https://support.flawlessthemes.com/hc/3570365436/category/67" target="blank">Click here</a> to access our Knowledge Base.
								</div>
							</div>
							<div class="tab-pane fade" id="bl-pills-4" role="tabpanel" aria-labelledby="bl-pills-4-tab">
								<form method="post" action="options.php">
									...
									settings_fields('hotel_inn_theme_option_group');
									do_settings_sections('hotel_inn_theme_options');
									submit_button();
									?>
	...
	<script>
		...
		jQuery(document).ready(function($) {
			...
			<?php 
				if (isset($_POST) && isset($_POST['hotel_inn_free_back_top_top_enable_form'])) {
...
function handle_post__request()
{
	if (isset($_POST) && isset($_POST['hotel_inn_free_back_top_top_enable_form'])) {
		...
		if (!empty($_POST['back_top_top_enable'])) {
			update_option('hotel_inn_free_back_top_top_enable', $_POST['back_top_top_enable']);
		} else {
			update_option('hotel_inn_free_back_top_top_enable', 0);
		}
	}
	if (isset($_POST) &&  isset($_POST['reset'])) {
		try {
			...
			foreach ($post_types as $t_key => $t_item) {
				update_option('hotel_inn_dslc_' . $t_key, 0);
			}

			update_option('hotel_inn_dslc_bread_all', 0);
			foreach ($post_types as $t_key => $t_item) {
				if (isset($_POST['hotel_inn_dslc_' . $t_key])) {
					update_option('hotel_inn_dslc_' . $t_key, 1);
				}
			}
			if (isset($_POST['hotel_inn_dslc_bread_all'])) {
				update_option('hotel_inn_dslc_bread_all', 1);
			}
			if (isset($_POST['hotel_inn_dslc_breadcrumb_show'])) {
				update_option('hotel_inn_dslc_breadcrumb_show', 'on');
			} else {
				update_option('hotel_inn_dslc_breadcrumb_show', 'off');
			}
		} catch (\Throwable $th) {
		}
	}
...
{
	// Home page link
	echo '<a href="' . home_url('/') . '">' . __('Home', 'hotel-inn') . '</a> <span class="separator">/</span> ';
	...
	if (is_singular()) {
		...
		echo '<span class="current">' . __('Search results for: ', 'hotel-inn') . get_search_query() . '</span>';
	} elseif (is_404()) {
		// Display 404 message
		echo '<span class="current">' . __('404 Not Found', 'hotel-inn') . '</span>';
	}
	echo "
	<style>.separator {
		margin: 0 5px;
...
{
	if (get_option('hotel_inn_dslc_breadcrumb_show') == 'on') {
		if (is_single()) {
			$post_type = get_post_type();
			if (get_option('hotel_inn_dslc_' . $post_type) == 1) {
				$already_set = 1;
				if (function_exists('custom_breadcrumbs')) custom_breadcrumbs();
			}
		} elseif (is_page()) {
			if (get_option('hotel_inn_dslc_page') == 1) {
				$already_set = 1;
				if (function_exists('custom_breadcrumbs')) custom_breadcrumbs();
			}
		}
		if (get_option('hotel_inn_dslc_bread_all') == 1 && !isset($already_set)) {
			if (function_exists('custom_breadcrumbs')) custom_breadcrumbs();
		}
	}
}
