...
if ( ! function_exists( 'hotel_and_travel_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time.
	 */
	function hotel_and_travel_posted_on() {
...
if ( ! function_exists( 'hotel_and_travel_posted_by' ) ) :
	/**
	 * Prints HTML with meta information for the current author.
	 */
	function hotel_and_travel_posted_by() {
		$byline = sprintf(
			/* translators: %s: post author. */
			esc_html_x( 'By %s', 'post author', 'hotel-and-travel' ),
...
if ( ! function_exists( 'hotel_and_travel_post_thumbnail' ) ) :
	/**
	 ...
	 */
	function hotel_and_travel_post_thumbnail() {
			...
			<figure class="post-thumbnail">
				<a href="<?php the_permalink(); ?>">
					<?php if( has_post_thumbnail() ) { ?>
						<?php the_post_thumbnail( 'hotel_and_travel_popular_posts', array( 'itemprop' => 'image' ) );
						...
						hotel_and_travel_get_fallback_svg( 'hotel_and_travel_popular_posts' );
					} ?>
				</a>
			</figure>
...
if( ! function_exists( 'hotel_and_travel_get_posts' ) ) :
	/**
	 * Fuction to list Custom Post Type
	*/
	function hotel_and_travel_get_posts( $post_type = 'post', $slug = false ){    
		$args = array(
			'posts_per_page'   => -1,
			'post_type'        => $post_type,
			'post_status'      => 'publish',
		...
		$post_options = array();
		$post_options[''] = __( ' -- Choose -- ', 'hotel-and-travel' );
...
if ( ! function_exists( 'hotel_and_travel_tag' ) ) :
/**
 * Prints tags
 */
function hotel_and_travel_tag(){
	// Hide category and tag text for pages.
	if ( 'post' === get_post_type() ) {
		$tags_list = get_the_tag_list( '', ' ' );
		if ( $tags_list ) {
			/* translators: 1: list of tags. */
			printf( '<div class="cat-tags" itemprop="about">' . esc_html__( '%1$sTags:%2$s %3$s', 'hotel-and-travel' ) . '</div>', '<span class="tags-title">', '</span>', $tags_list );
		}
	}
}
endif;

if( ! function_exists( 'hotel_and_travel_get_svg_icons' ) ) :
	/**
	 * Fuction to list svg
	*/
	function hotel_and_travel_get_svg_icons(){    
		...
		// Initate an empty array
		$svg_options = array();
		$svg_options[''] = __( ' -- Choose -- ', 'hotel-and-travel' );
...
if ( ! function_exists( 'hotel_and_travel_site_branding' ) ) :
    /**
     * Site Branding
     */
    function hotel_and_travel_site_branding(){
        ?>
        <div class="site-branding" itemscope itemtype="https://schema.org/Organization">
            ...
            $hotel_and_travel_description = get_bloginfo( 'description', 'display' );
            if ( $hotel_and_travel_description || is_customize_preview() ) :
                ?>
                <p class="site-description" itemprop="description"><?php echo $hotel_and_travel_description; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></p>
            <?php endif; ?>
            </div>
        </div><!-- .site-branding -->
        <?php
...
if ( ! function_exists( 'hotel_and_travel_primary_navigation' ) ) :
    /**
    * Primary Navigation
    */
    function hotel_and_travel_primary_navigation( $schema = true ){
                ...
                <?php
                    wp_nav_menu(
                        array(
                            ...
                            'fallback_cb'     => 'hotel_and_travel_primary_menu_fallback',
                        )
                    );
                ?>
            </nav>
...
if( ! function_exists( 'hotel_and_travel_social_links' ) ) :
	/**
	 * Social Links 
	*/
    function hotel_and_travel_social_links( $echo = true ){
        
        $social_links = hotel_and_travel_get_setting( 'social_links' );
        if( $social_links && $echo ){ ?>
        <ul class="social-links">
            <?php 
            foreach( $social_links as $link ){
                $new_tab = isset( $link['hotel_and_travel_checkbox'] ) && $link['hotel_and_travel_checkbox'] ? '_blank' : '_self';
                if( isset( $link['hotel_and_travel_link'] ) ){ ?>
                <li>
                    <a href="<?php echo esc_url( $link['hotel_and_travel_link'] ); ?>" target="<?php echo esc_attr( $new_tab ); ?>" rel="nofollow noopener">
                        <?php echo wp_kses( hotel_and_travel_social_icons_svg_list( $link['hotel_and_travel_icon'] ), hotel_and_travel_kses_extended_ruleset() ); ?>
                    </a>
                </li>	   
                <?php
                } 
...
if( ! function_exists( 'hotel_and_travel_header_search' ) ) :
/**
 * Header search
 */
function hotel_and_travel_header_search(){

$enable_search = hotel_and_travel_get_setting( 'enable_search', false );
if ( $enable_search ) { ?>
	<div class="header-search">
		<button class="header-search-icon" aria-label="<?php esc_attr_e( 'search form toggle', 'hotel-and-travel' ); ?>" data-toggle-target=".search-modal" data-toggle-body-class="showing-search-modal" data-set-focus=".search-modal .search-field" aria-expanded="false">
		...
		<div class="header-search-form search-modal cover-modal" data-modal-target-string=".search-modal">
			<div class="header-search-inner-wrap">'
				<?php get_search_form(); ?>
				<button aria-label="<?php esc_attr_e( 'search form close', 'hotel-and-travel' ); ?>" class="close" data-toggle-target=".search-modal" data-toggle-body-class="showing-search-modal" data-set-focus=".search-modal .search-field" aria-expanded="false"></button>
			</div>
		</div>
	</div><!-- .header-seearch -->
<?php } 
...
if( ! function_exists( 'hotel_and_travel_primary_menu_fallback' ) ) :
/**
 * Fallback for primary menu
*/
function hotel_and_travel_primary_menu_fallback(){
	?>
        <div class="primary-menu-container">
			<ul id="primary-menu" class="nav-menu">
				<li><a href="<?php echo esc_url( admin_url( 'nav-menus.php' ) ); ?>"><?php echo esc_html__( 'Click here to add a menu', 'hotel-and-travel' ); ?></a></li>
			</ul>
        </div>
	<?php 
}
...
if( ! function_exists( 'hotel_and_travel_nav' ) ) :
/**
 * navigation
 */
function hotel_and_travel_nav(){
	if( is_singular() ){ 
								...
								<article class="post">
									<figure class="post-thumbnail">
										...
										if( $prev_img ){
											...
											hotel_and_travel_get_fallback_svg( 'thumbnail' );
										}?>
									</figure>
								<div class="pagination-details">
									<span class="meta-nav"><?php echo esc_html__( 'Previous', 'hotel-and-travel' ); ?></span>
						...
						<div class="nav-next">
							<a href="<?php the_permalink( $next_post->ID ); ?>" rel="next">
								<article class="post">
									...
									<div class="pagination-details">
										<span class="meta-nav"><?php echo esc_html__( 'Next', 'hotel-and-travel' ); ?></span>
									<header class="entry-header">
...
if ( ! function_exists( 'hotel_and_travel_category' ) ) :
/**
 * Prints categories
 */
function hotel_and_travel_category(){
    // Hide category and tag text for pages.
    if ( 'post' === get_post_type()) {
...
function hotel_and_travel_tag(){
    ...
    if ( 'post' === get_post_type() ) {
        ...
        if ( $tags_list ) {
            /* translators: 1: list of tags. */
            printf( '<div class="tags">' . esc_html__( '%1$sTags:%2$s %3$s', 'hotel-and-travel' ) . '</div>', '<span>', '</span>', $tags_list );
        }
    }
}
endif;
...
if( ! function_exists( 'hotel_and_travel_get_image_sizes' ) ) :
/**
 * Get information about available image sizes
 */
function hotel_and_travel_get_image_sizes( $size = '' ) {
...
if ( ! function_exists( 'hotel_and_travel_get_fallback_svg' ) ) :    
/**
 * Get Fallback SVG
*/
function hotel_and_travel_get_fallback_svg( $post_thumbnail ) {
    if( ! $post_thumbnail ){
        return;
    }


    $image_size = hotel_and_travel_get_image_sizes( $post_thumbnail );
...
if ( ! function_exists( 'hotel_and_travel_author_meta' ) ) :
    /**
     * Author info for Banner and Editor Picks
     */
    function hotel_and_travel_author_meta(){ ?>
        <div class="auth-details">
            <div class="author-desc">            
                <div class="author-details">
                    <?php echo get_avatar( get_the_author_meta( 'ID' ), 28, '', 'author' ); ?>
                    <div class="author-name">
                        <?php hotel_and_travel_posted_by(); ?>
                    </div>
                </div>
                <span class="date">
                    <?php hotel_and_travel_posted_on(); ?>
                </span>
...
if ( ! function_exists( 'hotel_and_travel_single_author_meta' ) ) :
    /**
     * Author info for Single Post Banner
     */
    function hotel_and_travel_single_author_meta(){   
        $ed_author          = hotel_and_travel_get_setting( 'enable_post_author', false );
        $ed_read_calc       = hotel_and_travel_get_setting( 'enable_post_read_calc', false );
		$ed_date            = hotel_and_travel_get_setting( 'enable_post_date', false );
		$ed_comments        = hotel_and_travel_get_setting( 'enable_banner_comments', false );     
        ?>
        <div class="auth-details">
            <div class="author-desc">
                <?php if( ! $ed_author ){ ?> 
                ...
                <?php } if( ! $ed_read_calc ) hotel_and_travel_single_reading_calc( get_post( get_the_ID() )->post_content ); ?>
            </div>
        </div>
    <?php    
    }
...
if( ! function_exists( 'hotel_and_travel_get_posts_list' ) ) :
/**
 * Returns Latest & Related 
*/
function hotel_and_travel_get_posts_list( $status ){
    global $post;
    ...
    switch( $status ){
		...
		$reltitle         = __( 'You might also like', 'hotel-and-travel' );
		$class            = 'recent-posts';
		$image_size       = 'hotel_and_travel_popular_posts';
        break;
        ...
        $args['orderby']        = 'rand';
		$reltitle         		= hotel_and_travel_get_setting( 'related_post_title', __( 'You might also like', 'hotel-and-travel' ) );
		$image_size       		= 'hotel_and_travel_archive';
            ...
            <div class="section-grid">
                ...
                <article class="post">
                        ...
                        <a href="<?php the_permalink(); ?>" class="post-thumbnail">
                            <?php
                                if( has_post_thumbnail() ){
                                    ...
                                    hotel_and_travel_get_fallback_svg( $image_size );//fallback
                                }
                            ?>
                        </a>
                    </div>
                    <header class="entry-header">
                        ...
                        <?php hotel_and_travel_author_meta(); ?>                 
                    </header>
                </article>
                <?php } ?>    		
            </div>
...
if( ! function_exists( 'hotel_and_travel_sidebar_layout' ) ) :
/**
 * Return sidebar layouts for pages/posts
 */
function hotel_and_travel_sidebar_layout(){
    global $post;
	$return      = false;
	$page_layout = hotel_and_travel_get_setting( 'page_sidebar_layout'); //Default Layout Style for Pages
	$post_layout = hotel_and_travel_get_setting( 'post_sidebar_layout'); //Default Layout Style for Posts
    $layout      = hotel_and_travel_get_setting( 'layout_style'); //Default Layout Style for Archive and Search Pages
    $woo_layout  = hotel_and_travel_get_setting( 'woo_sidebar_layout'); //Woo Sidebar Layout
    $product_layout = hotel_and_travel_get_setting( 'product_sidebar_layout'); //prod Sidebar Layout
    $checkout_layout = hotel_and_travel_get_setting( 'checkout_sidebar_layout'); //prod Sidebar Layout
...
if ( ! function_exists( 'hotel_and_travel_author_box' ) ) :
    /**
     * Author Box for Single Post and Archive Page
     */
    function hotel_and_travel_author_box(){
...
if( ! function_exists( 'hotel_and_travel_mobile_navigation' ) ) :
/**
 * Mobile Navigation
*/
function hotel_and_travel_mobile_navigation(){ 
    ?>
            ...
            <div class="container">
                <div class="mob-nav-site-branding-wrap">
                    <div class="header-center">
                        <?php hotel_and_travel_site_branding(); ?>
                    </div>
                ...
                <nav id="mobile-site-navigation" class="main-navigation mobile-navigation">        
                    <div class="primary-menu-list main-menu-modal cover-modal" data-modal-target-string=".main-menu-modal">                  
                        ...
                        <div class="mobile-social-wrap">
                            <?php if( hotel_and_travel_social_links( false ) ){
                                echo '<div class="header-left"><div class="header-social">';
                                hotel_and_travel_social_links();
                                echo '</div></div>';
                            } ?>  
                        </div>
                        <div class="mobile-menu" aria-label="<?php esc_attr_e( 'Mobile', 'hotel-and-travel' ); ?>">
                            <?php
                                hotel_and_travel_primary_navigation( false );
                            ?>
                        </div>
                    </div>
                    <?php
                      if (hotel_and_travel_get_setting('enable_mobile_search')){
...
if( ! function_exists( 'hotel_and_travel_comment' ) ) :
	/**
	 * Callback function for Comment List *
	 * 
	 * @link https://codex.wordpress.org/Function_Reference/wp_list_comments 
	 */
	function hotel_and_travel_comment( $comment, $args, $depth ){
		if ( 'div' == $args['style'] ) {
			$tag = 'div';
			$add_below = 'comment';
		} else {
		...
		<?php endif; ?>
			...
			<footer class="comment-meta">
                ...
                <div class="author-details-wrap"><!-- .comment-author vcard -->
                    <?php printf( __( '<b class="fn" itemprop="creator" itemscope itemtype="http://schema.org/Person">%s <span class="says">says:</span></b>', 'hotel-and-travel' ), get_comment_author_link() ); ?>
                    <div class="comment-meta-data">
                        <a href="<?php echo esc_url( htmlspecialchars( get_comment_link( $comment->comment_ID ) ) ); ?>">
                            <time itemprop="commentTime" datetime="<?php echo esc_attr( get_gmt_from_date( get_comment_date() . get_comment_time(), 'Y-m-d H:i:s' ) ); ?>"><?php printf( esc_html__( '%1$s at %2$s', 'hotel-and-travel' ), get_comment_date(),  get_comment_time() ); ?></time>
                        </a>
                    </div>
                    <?php if ( $comment->comment_approved == '0' ) : ?>
                        <em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'hotel-and-travel' ); ?></em>
                        <br />
                    <?php endif; ?>
                    ...
                    <div class="reply">
                        <?php comment_reply_link( array_merge( $args, array( 'reply_text' => __('Reply', 'hotel-and-travel'), 'add_below' => $add_below, 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
                    </div>
                </div>
			</footer> 
		<?php if ( 'div' != $args['style'] ) : ?>
...
if( ! function_exists( 'hotel_and_travel_single_reading_calc' ) ) :
    /**
     * Reading calculation
     */
    function hotel_and_travel_single_reading_calc( $content ){ 
        $en_read       = hotel_and_travel_get_setting( 'enable_post_read_calc' );
        $read_calc     = hotel_and_travel_get_setting( 'read_words_per_minute' );
   $total_word = str_word_count(strip_tags($content));
    $m = floor($total_word / $read_calc);
    $s = floor($total_word % $read_calc / ($read_calc / 60));
    $estimateTime = $m . esc_html__(' minute', 'hotel-and-travel') . ($m == 1 ? '' : 's') . ', ' . $s . esc_html__(' second','hotel-and-travel') . ($s == 1 ? '' : 's');

        if ( ! $en_read ) { ?>
            <div class="time">
                <?php echo wp_kses_post( $estimateTime ).esc_html__(' Read', 'hotel-and-travel'); ?>
            </div>
        <?php }
    }
endif;
...
if( ! function_exists( 'hotel_and_travel_footer_site_info' ) ) :
/**
 * Footer site info
*/
function hotel_and_travel_footer_site_info(){ 
	echo '<div class="site-info">';
		hotel_and_travel_get_footer_copyright();
	echo '</div>';
}
endif;


if( ! function_exists( 'hotel_and_travel_get_related_posts' ) ) :
    /**
     * Related post
    */
    function hotel_and_travel_get_related_posts(){ ?>
        <div class="additional-post">
            <?php hotel_and_travel_get_posts_list( 'related' ); ?>
        </div>
        <?php 
    }
endif;

if( ! function_exists( 'hotel_and_travel_get_comments' ) ) :
    /**
     * Comments
    */
    function hotel_and_travel_get_comments(){ 
...
if( ! function_exists( 'hotel_and_travel_kses_extended_ruleset' ) ) :

function hotel_and_travel_kses_extended_ruleset() {
...
if ( ! function_exists( 'hotel_and_travel_social_icons_svg_list' ) ) :    
	/**
	 * Get SVG Image
	*/
	function hotel_and_travel_social_icons_svg_list( $social ){
...
if( ! function_exists( 'hotel_and_travel_is_woocommerce_activated' ) ) :
	/**
	 * Query WooCommerce activation
	 */
	function hotel_and_travel_is_woocommerce_activated() {
		return class_exists( 'woocommerce' ) ? true : false;
	}
endif;

if( ! function_exists( 'hotel_and_travel_is_btnw_activated' ) ) :
	/**
	 * Is gradientthemes Email Newsletters active or not
	*/
	function hotel_and_travel_is_btnw_activated(){
...
function hotel_and_travel_post_grid( $loop, $max_height, $title, $layout, $excerpt, $colums ){
		...
		while( $loop->have_posts() ) : $loop->the_post();
			...
			<div class="<?php echo esc_attr($colums); ?> post-compact" >
		  		<div class="background-img" >
					<a  href="<?php the_permalink(); ?>"><?php the_post_thumbnail('hotel-and-travel-medium'); ?></a>
                    <div class="post-compact-container" >
						<div class="portfolio-content" >
							...
							<?php hotel_and_travel_print_post_cat($post_id); ?>												
						</div>
			 ...
			 <div class="post-content" >
		  		<div class="post-compact-container">                    
					...
					<div class="layout-list portfolio-content" >
						...
						<p><?php echo wp_kses_post( hotel_and_travel_trim_content($excerpt , $post) ); ?></p>
						<?php endif; ?>
						
						<a class="more-link" href="<?php the_permalink(); ?>"><?php echo esc_html__("Read More", "hotel-and-travel"); ?></a>
					</div>
				</div>				
			</div>
			<?php
			...
			<div class="<?php echo esc_attr($colums); ?>  post-list">
			 <div class="post-content vertical-center">		  
		  		<div class="<?php if($layout == 4) echo 'small-list'; ?> post-img" >
					...
					<a  href="<?php the_permalink(); ?>"><?php the_post_thumbnail('hotel-and-travel-medium-square'); ?></a>
					<?php hotel_and_travel_print_post_cat($post_id); ?>
					</div>
				</div>
 ...
 * 
 */
function hotel_and_travel_print_post_cat ( $post_id ){
...
class hotel_and_travel_cat	{
	public $term_id = '';
	public $name = '';
}

if ( ! function_exists( 'hotel_and_travel_trim_content' ) ) :
	/**
	 ...
	 */
	function hotel_and_travel_trim_content( $length = 40, $post_obj = null ) {
		global $post;
		if ( is_null( $post_obj ) ) {
			$post_obj = $post;
		}
	   ...
	   return apply_filters( 'hotel_and_travel_trim_content', $trimmed_content );
	}
endif;
...
function hotel_and_travel_list_popular_taxonomies($taxonomy, $title, $number = 5)
{
